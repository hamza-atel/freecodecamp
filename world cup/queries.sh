...
echo "$($PSQL "select name from teams right join games on teams.team_id=games.winner_id where round like 'Final' and year=2018;")"
...
echo "$($PSQL "select year, name from teams right join games on teams.team_id=games.winner_id where round like 'Final' order by year;")"
